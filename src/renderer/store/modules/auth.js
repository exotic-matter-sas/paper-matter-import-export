...
const mutations = {
  ...
  SAVE_AUTHENTICATION_DATA(
    ...
    state.accessTokenExpireTimestamp =
      new Date().getTime() + parseInt(accessTokenExpiresIn) * 1000;
...
const actions = {
  refreshAccessToken({ commit, state, dispatch }, apiClient) {
    ...
    if (currentDate < tokenExpirationDate) {
      log.debug(
        "access token is still valid, refresh should not be needed (unless it was revoked by server)"
      );
  ...
  disconnectUser({ commit, state }, { apiClient, reason = null }) {
    if (
      ...
      return apiClient
        .revokeToken(state.accessToken, "access_token")
