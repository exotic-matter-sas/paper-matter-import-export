...
export const reportTools = {
  HtmlReport: class HtmlReport {
    ...
    render() {
      ...
      const style =
        "<style>body{background: #f8f9fa} thead{font-weight: bold} td{padding-right: 10px}</style>";

      const tableHeader = `<tr>${this.columnsTitles
        .map((title) => "<td>" + title + "</td>")
        .join("")}</tr>`;
      ...
      this.htmlReport = `<!DOCTYPE html><html lang="en"><head><title>Error report ${this.reportDate.toLocaleTimeString(
        navigator.language,
        { hour: "2-digit", minute: "2-digit" }
      )}</title><meta charset="utf-8">${style}</head><body>${table}</body></html>`;
      log.debug("html report rendered");
    }

    save() {
      log.debug("saving error report to disk");
      const fileName = `error_report_${this.reportDate
        .toISOString()
        .replace(/\:/g, "-")}.html`;
