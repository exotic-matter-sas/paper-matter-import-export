      ...
      <b-col>
        <b-form-group>
          ...
          <label
            class="d-block"
            id="update-source"
            :title="$t('exportTab.sourceOnlyRootTitle')"
    ...
    <b-row>
      <b-col>
        ...
        >
          ...
          <label
            class="d-block border border-success rounded"
            id="update-destination"
            :title="savedExportDestination"
...
export default {
  ...
  methods: {
    setDestinationFolder() {
      ...
      remote.dialog
        .showOpenDialog(win, {
          ...
          defaultPath:
            this.savedExportDestination === null
              ? os.homedir()
        ...
        .then((selectionObject) => {
          if (selectionObject.canceled) {
            ...
            vi.$store.commit(
              "export/SET_EXPORT_DESTINATION",
              selectionObject.filePaths[0]
            );
          }
        });
    },
      ...
      try {
        ...
        if (!this.exportFolderName) {
          try {
            ...
            fs.mkdirSync(
              path.join(this.savedExportDestination, this.exportFolderName),
              { recursive: true }
            );
          } catch (e) {
        ...
        try {
          ...
          log.error(
            "ignoring metadata export, an error occurred during CSV file initialization:\n",
            e
          );
        ...
        if (vi.docsToExport.length === 0) {
          ...
          await vi
            .listAllDocuments(1, 0, sourceFolderId)
            .then((allDocuments) => {
              vi.$store.commit(
            ...
            .catch((e) => {
              log.error(
                "aborting export, error occurred during documents listing:\n",
                e
              );
        ...
        while (
          ...
          docDirAbsolutePath = this.getDocDirAbsolutePath(
          ...
          await fs.promises
            .mkdir(docDirAbsolutePath, { recursive: true })
            .catch((e) => {
              log.error(
                "skipping document, parent folder creation failed:\n",
                e
              );
              vi.$store.commit(
                "export/MOVE_FIRST_DOC_FROM_EXPORT_TO_ERROR",
        ...
        vi.notifyExportEnd(
          totalCount,
          path.join(vi.savedExportDestination, vi.exportFolderName)
        );
      } catch (e) {
    ...
    listAllDocuments(startPage, currentDocumentCount, level) {
      ...
      return this.$api
        .listDocuments(vi.accessToken, startPage, level)
        .then(async (response) => {
          // Display and update progressModal
          vi.$emit("event-exporting", {
      ...
      return (
        vi.$api
          .downloadDocumentAsArrayBuffer(
            vi.accessToken,
            serializedDocument.download_url
          )
              ...
              .then((computedMd5) => {
                if (serializedDocument.md5 === computedMd5) {
                  ...
                  return Promise.reject(
                    "File md5 mismatch, file seems to have been corrupted during download"
                  );
                }
              });
          })
            ...
            return fs.promises
              ...
              .catch(async (error) => {
                ...
                if ("code" in error && error.code === "EEXIST") {
                  ...
                  const docPathHash = await vi.hashString({
                    algorithm: "md5",
                    string: docAbsolutePath,
                  });
                  let fileCount;

                  if (docPathHash in vi.duplicatedFilePathCount) {
                    fileCount = vi.duplicatedFilePathCount[docPathHash] + 1;
                    vi.$store.commit("export/SET_DUPLICATED_FILE_PATH_COUNT", [
                      docPathHash,
                      fileCount,
                    ]);
                  } else {
    ...
    displayFatalError(error) {
      ...
      if (error.name === "PMISetupError") {
        errorTitle = this.$t(
          `exportTab.expectedFatalErrors.${error.message}.title`
        );
        errorMessage = this.$t(
          `exportTab.expectedFatalErrors.${error.message}.message`
        );
        errorDetail = this.$t(
          `exportTab.expectedFatalErrors.${error.message}.detail`
        );
      }
    ...
    displayExportErrorPrompt(errorCount, optionalMention = "") {
      ...
      remote.dialog
        .showMessageBox(win, {
          type: "error",
          title: this.$tc(
            "exportTab.errorExportTitle",
            this.exportDocsInError.length
          ),
          message: this.$tc(
            "exportTab.errorExportMessage",
            this.exportDocsInError.length
          ),
          detail: this.$tc(
            "exportTab.errorExportDetail",
            this.exportDocsInError.length,
            { optionalMention }
          ),
    ...
    notifyExportEnd(totalCount, exportFolderPath) {
      ...
      const mentionsList = [
        this.metadataExportSkipped
          ? this.$t("exportTab.metadataNotExportedMention")
          : "",
          ...
          remote.dialog
            .showMessageBox(win, {
              ...
              message: this.$tc(
                ...
                totalCount - this.docsToExport.length,
                { joined_mentions: joinedMentions }
              ),
